...
class ComicDataset(utils.Dataset):
    ...
    def load_comic(self, dataset_dir, subset):
        ...
        self.add_class("comic", 1, "balloon")
        ...
        for i in data[2]:
            ...
            for annotation in data[3]:
                if(img_id == annotation['image_id'] and annotation['category_id'] == 3):
...
def infer(model, dataset_dir, subset):
    ...
    dataset.load_comic(dataset_dir, subset)
    dataset.prepare()
    ...
    for image_id in dataset.image_ids:
        ...
        if mask.shape[0] > 0:
            output = np.where(mask, image)

        else:
            output = image

        file_name = "inf_{}.png".format(image_id)

        skimage.io.imsave(file_name,output)
...
if __name__ == '__main__':
    ...
    if args.command == "train":
        ...
        model = modellib.MaskRCNN(mode="inference", config=_InfConfig,
                                  model_dir=args.logs)
        # model_path = model_inference.find_last()
        # print("Loading weights from ", model_path)
        # model_inference.load_weights(model_path, by_name=True, exclude=[
